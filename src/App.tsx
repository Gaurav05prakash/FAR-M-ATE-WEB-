...
function FarmateWorkspace({ user, onLogout }: { user: User; onLogout: () => void }) {
  ...
  // Interactive Onboarding Demo Tour (first-time sign-in automated activation)
  const [isDemoTourOpen, setIsDemoTourOpen] = useState(false);

  useEffect(() => {
    if (!user) return;
    const tourKey = `farmate_demo_seen_${user.id || user.email}`;
    const hasSeenTour = localStorage.getItem(tourKey) === 'true';

    // If first-time sign-in, activate demo after dashboard mounts
    if (!hasSeenTour) {
  ...
  return (
    <div className="min-h-screen w-full max-w-full overflow-x-hidden bg-[#f8faf9] text-neutral-900 flex flex-col font-sans selection:bg-emerald-500 selection:text-white">
      ...
      {/* Interactive First-Time Voice-Guided Onboarding Demo Tour */}
      <InteractiveDemoTour
        isOpen={isDemoTourOpen}
        onClose={() => setIsDemoTourOpen(false)}
        language={language}
        onSelectLanguage={handleLanguageChange}
        onOpenSettingsModal={() => setIsAuthModalOpen(true)}
