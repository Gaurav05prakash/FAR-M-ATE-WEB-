    ...
    <header className="sticky top-0 z-30 bg-white/95 backdrop-blur-md border-b border-neutral-200/80 shadow-2xs">
      <div className="w-full px-3 sm:px-5 lg:px-6 h-16 flex items-center justify-between gap-2 sm:gap-4">
        ...
        <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
          ...
          {/* GPS Location Badge: Synced with GPS & updates everywhere */}
          <button
            ...
            title={
              weather
                ? `Detected Farm Location: ${weather.location}. Click to re-detect GPS coordinates.`
                : 'Click to detect GPS location'
            }
            className="flex items-center gap-1 sm:gap-1.5 bg-white hover:bg-emerald-50/80 border border-neutral-200 hover:border-emerald-300 rounded-full px-2 sm:px-2.5 py-1.5 shadow-2xs shrink-0 cursor-pointer transition select-none group"
          >
            <MapPin className="w-3.5 h-3.5 text-emerald-600 shrink-0 group-hover:scale-110 transition-transform" />
            <span className="text-xs font-bold text-neutral-800 max-w-[70px] sm:max-w-[120px] md:max-w-[150px] truncate">
              {weather?.location || user.farmProfile?.location || 'Detecting Location...'}
            </span>
            {weatherLoading && <RefreshCw className="w-3 h-3 text-emerald-600 animate-spin shrink-0 ml-0.5" />}
          </button>

          {/* Rescaled Weather Button */}
          <button
